...
void Interrupt_RSPI0_SPTI0(void)
{
	...
	if (rpdl_SPI_method[0] == SPI_USING_IRQ)
	{
        ...
        if (rpdl_SPI_frame_total[0] <= 4)
        {
        	/* If last sequence loop */
			if (rpdl_SPI_tx_sequence_counter[0] == rpdl_SPI_sequence_count[0] - 1)
			{
		...
		else
		{
			/* If last frame */
			if ((rpdl_SPI_tx_sequence_counter[0] == rpdl_SPI_sequence_count[0] - 1) &&\
				(rpdl_SPI_tx_frame_counter[0] == rpdl_SPI_frame_total[0] - 1))
			{
				/* Disable transmit interrupts */
				RSPI0.SPCR.BIT.SPTIE = 0;
			}
		...
		/* Increment the frame counter */
		rpdl_SPI_tx_frame_counter[0] += frame_count;
...
void Interrupt_RSPI1_SPTI1(void)
{
	...
	if (rpdl_SPI_method[1] == SPI_USING_IRQ)
	{
        ...
        if (rpdl_SPI_frame_total[1] <= 4)
        {
        	/* If last sequence loop */
			if (rpdl_SPI_tx_sequence_counter[1] == rpdl_SPI_sequence_count[1] - 1)
			{
				/* Disable transmit interrupts */
		...
		else
		{
			/* If last frame */
			if ((rpdl_SPI_tx_sequence_counter[1] == rpdl_SPI_sequence_count[1] - 1) &&\
				(rpdl_SPI_tx_frame_counter[1] == rpdl_SPI_frame_total[1] - 1))
			{
				/* Disable transmit interrupts */
				RSPI1.SPCR.BIT.SPTIE = 0;
			}
		...
		/* Increment the frame counter */
		rpdl_SPI_tx_frame_counter[1] += frame_count;
