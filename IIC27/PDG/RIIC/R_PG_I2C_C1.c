...
* Limitations  : 
******************************************************************************
* History : 27.02.2014 Version Description
*         :   
******************************************************************************/
...
* Include      : 
*
* Declaration  : bool R_PG_I2C_MasterReceive_C1(uint16_t slave, uint8_t * data, uint16_t count)
*
* Function Name: R_PG_I2C_MasterReceive_C1
*
* Description  : マスタのデータ受信
*
* Arguments    : uint16_t slave : スレーブアドレス
...
* Details      : 詳細についてはリファレンスマニュアルを参照してください。
******************************************************************************/
bool R_PG_I2C_MasterReceive_C1(uint16_t slave, uint8_t * data, uint16_t count)
{
	if( data == 0 )
	{
		return false;
	...
	return R_IIC_MasterReceive(
		1,
		PDL_IIC_DMAC_DTC_TRIGGER_DISABLE,
		slave,
		data,
		count,
		PDL_NO_FUNC,
...
* Include      : 
*
* Declaration  : bool R_PG_I2C_MasterSend_C1(uint16_t slave, uint8_t * data, uint16_t count)
*
...
* Details      : 詳細についてはリファレンスマニュアルを参照してください。
******************************************************************************/
bool R_PG_I2C_MasterSend_C1(uint16_t slave, uint8_t * data, uint16_t count)
{
	if( data == 0 )
	{
		return false;
	...
	return R_IIC_MasterSend(
		1,
		PDL_IIC_START_ENABLE | PDL_IIC_STOP_ENABLE | PDL_IIC_DMAC_DTC_TRIGGER_DISABLE,
		slave,
		data,
		count,
		PDL_NO_FUNC,
...
* Include      : 
*
* Declaration  : bool R_PG_I2C_MasterSendWithoutStop_C1(uint16_t slave, uint8_t * data, uint16_t count)
*
...
* Details      : 詳細についてはリファレンスマニュアルを参照してください。
******************************************************************************/
bool R_PG_I2C_MasterSendWithoutStop_C1(uint16_t slave, uint8_t * data, uint16_t count)
{
	if( data == 0 )
	{
		return false;
	...
	return R_IIC_MasterSend(
		1,
		PDL_IIC_START_ENABLE | PDL_IIC_STOP_DISABLE | PDL_IIC_DMAC_DTC_TRIGGER_DISABLE,
		slave,
		data,
		count,
		PDL_NO_FUNC,
